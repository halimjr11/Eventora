...
import coil.load
import com.halimjr11.eventora.databinding.ItemFinishedEventBinding
...
class FinishedAdapter : ListAdapter<EventDomain, FinishedAdapter.FinishedViewHolder>(
    ...
    inner class FinishedViewHolder(private val binding: ItemFinishedEventBinding) :
        RecyclerView.ViewHolder(binding.root) {
        fun bind(data: EventDomain) = binding.run {
            tvEventTitle.text = data.name
            tvEventDesc.text = data.summary
            imgThumb.load(data.imageLogo)
    ...
    override fun onCreateViewHolder(
        ...
        ItemFinishedEventBinding.inflate(
